...
return ( <div className="min-h-screen bg-[#0a0a0f]"> <div className="max-w-6xl mx-auto px-4 py-8"> {/* Navigation */} <button onClick={() => router.back()} className="flex items-center gap-2 text-cyan-400 hover:text-cyan-300 transition-colors mb-6"         >
    ...
    <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-2xl p-4 mb-8 flex items-center gap-4">
      ...
      <p className="text-sm text-cyan-100/80">
        {t.tradingPairs.followUpdates} <a href="https://t.me/t4ulife" target="_blank" rel="noopener noreferrer" className="text-white font-bold underline hover:text-cyan-400 transition-colors">{t.tradingPairs.telegramChannel}</a>.
      </p>
    </div>
    ...
    <div className="mt-12 p-8 border border-dashed border-slate-800 rounded-2xl text-center">
      ...
      <h3 className="text-white font-semibold mb-1">{t.tradingPairs.lookingForPair}</h3>
      <p className="text-sm text-slate-500">
        {t.tradingPairs.suggestAsset}
      </p>
    </div>
  </div>
</div>
