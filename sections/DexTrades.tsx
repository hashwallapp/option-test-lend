...
import React from "react"; import { ArrowLeft, Activity, History, Send, BarChart3, ExternalLink, MessageSquare, Smartphone, CheckCircle2 } from "lucide-react"; import { useRouter } from "next/navigation";

const DexTrades: React.FC = () => { const router = useRouter();

return ( <div className="min-h-screen bg-[#0a0a0f] text-slate-300"> <div className="max-w-6xl mx-auto px-6 py-12"> {/* Navigation */} <button onClick={() => router.back()} className="group flex items-center gap-2 text-cyan-400 hover:text-cyan-300 transition-all mb-10" > <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" /> <span className="text-sm font-medium">Back to Home</span> </button>
{/* Header */}
